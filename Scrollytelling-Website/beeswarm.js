...
// Group events by 4-year periods for clustering
const minYear = d3.min(validEvents, d => d.date.year);
const maxYear = d3.max(validEvents, d => d.date.year);

const eventsByPeriod = new Map();
validEvents.forEach(event => {
    const period = Math.floor((event.date.year - minYear) / 4);
    const startYear = minYear + period * 4;
    const endYear = Math.min(startYear + 3, maxYear);
...
eventCircles
    .on("mouseover", function(event, d) {
        ...
        if (d.incident.casualties.total > 0) {
            tooltipContent += `<strong>Casualties:</strong> ${d.incident.casualties.killed} killed, ${d.incident.casualties.injured} injured`;
        }
